...
describe('proteinDossier', () => {
  it('generates a workflow message that names the bridge and entry tools', () => {
    const args = proteinDossier.args!.parse({ identifier: 'TP53' });
    const messages = proteinDossier.generate(args);
  ...
  it('weaves the organism into the workflow when provided', () => {
